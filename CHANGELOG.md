...
- **Android Connect** — declare `ACCESS_NETWORK_STATE` / `ACCESS_WIFI_STATE` so ConnectivityManager network callbacks no longer throw SecurityException on every Connect
- **VPN establish harden** — session network-callback registration is best-effort and cannot tear down a successful TUN
...
- **WhatsApp** offline on TUN — `sniff_override_destination: true` so FakeIP dials sniffed domains (same path Telegram already needed)
- **Instagram** offline — stop Meta VPN HTTP-proxy exclusions so Instagram HTTPS uses `10809` like Play Store/Google (gVisor TUN exclusion path was blackholing the app)

### Removed

- Temporary Connect debug instrumentation from the 2.5.0.1 investigation
...
- Instagram **Direct** still offline after 2.3.2 — Meta hosts now **bypass VPN HTTP proxy** (`ProxyInfo` exclusion list) so MQTT uses TUN + real DNS; Play Store / Translate keep `10809`
...
- Android **Instagram Direct** still offline while WhatsApp/Telegram worked — removed VPN **`SetHttpProxy(10809)`** so Direct uses TUN (Meta MQTT was trapped on HTTP CONNECT); TUN FakeIP now sets **`sniff_override_destination: true`**
