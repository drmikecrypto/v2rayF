...
3. If connect fails, read the status message — the app tears down VPN so normal internet keeps working. Connect is green when SOCKS + (on Android TUN) HTTP `10809` pass; TUN gen204 is advisory (soft rebind if weak) — see [`tips/golden-app-matrix.md`](tips/golden-app-matrix.md).
